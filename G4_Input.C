...
namespace Input
{
  ...
  void ApplyEICBeamParameter(PHHepMCGenHelper *HepMCGen)
  {
    ...
    {
      std::cout << "Can not enable Enable::IP6 and Enable::IP8 at the same time!" << std::endl;
      gSystem->Exit(1);
    }
    if (Enable::IP6 == false and Enable::IP8 == false)
    {
      std::cout << "None of the possible EIC IPs were selected: Enable::IP6 and Enable::IP8 !" << std::endl;
      gSystem->Exit(1);
    }
...
namespace PYTHIA6
{
  std::string config_file = std::string(getenv("CALIBRATIONROOT")) + "/Generators/phpythia6_e18p275_MinPartonP10GeV.cfg";
}
...
namespace SARTRE
{
  std::string config_file = std::string(getenv("CALIBRATIONROOT")) + "/Generators/sartre.cfg";
}
